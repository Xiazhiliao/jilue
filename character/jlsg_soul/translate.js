...
const translates = {
	...
	jlsg_qugu: "曲顾",
	jlsg_qugu_info: `每回合限X次（X为你的体力上限），当任意角色于濒死状态外使用出牌阶段可以使用的基本牌或非延时锦囊牌指定目标后，若此牌的花色与任意角色使用的上一张牌不同，你可以将此牌的效果改为同类型且符合上一张牌花色的牌的效果，并令此牌无视防具、不能被响应且造成的伤害改为${get.poptip("jlsg_realDamage")} ，然后摸一张牌，若此牌造成了伤害，你摸一张牌，以此法获得的牌不计入手牌上限和次数限制。`,
