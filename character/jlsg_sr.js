	...
	skill: {
		...
		jlsg_tianshang: {
			...
			async content(event, trigger, player) {
				const {
					...
					skills = ["jlsg_huiqu", "jlsg_old_yiji", "jlsg_yiji"].filter(s => player.hasSkill(s));
		...
		jlsg_old_yingcai: {
			...
			content: function () {
				...
				if (!event.suit.includes(get.suit(event.cards2))) event.suit.push(get.suit(event.cards2));
		...
		jlsg_youdi: {
			...
			subSkill: {
				shaMiss: {
					...
					async cost(event, trigger, player) {
						event.result = await player
							.chooseToDiscard("是否发动【诱敌】？", [1, trigger.player.countCards("he")], "he")
							.set("ai", card => {
		...
		jlsg_paoxiao: {
			...
			async content(event, trigger, player) {
				await player.draw(1);
				const { result } = await player.chooseToUse().set("filterCard", (card, player, event) => {
			...
			filterTarget: function (card, player, target) {
				...
				if (ui.selected.targets.length == 1) {
					...
					if (cardName === "jiedao") {
						...
						return target.getEquip(1) && game.hasPlayer(shaTarget => target.canUse("sha", shaTarget, false));
					}
		...
		jlsg_chouxi: {
			...
			content: function () {
				...
				event.types.sort();
				var prompt = "弃置一张与展示牌类别均不同的牌,然后让" + get.translation(player) + "获得" + get.translation(event.cards1) + ",或受到来自" + get.translation(player) + "的1点伤害并获得其中一种类别的牌.";
				player.line(target);
				...
				target
					...
					.set("dialog", [prompt, "hidden", event.cards1])
		...
		jlsg_jianxiong2: {
			...
			content: function () {
				...
				if (event.list.length) {
					...
					event.current = current;
					player
						.chooseToDiscard(get.prompt2("jlsg_jianxiong", current))
						.set("ai", function (card) {
		...
		jlsg_zhonghou_global: {
			...
			filter: function (event, player) {
				...
				var criterion0 = game.hasPlayer(function (target) {
					return ((player == target && (target.hasSkill("jlsg_zhonghou") || target.hiddenSkills.includes("jlsg_zhonghou"))) || (player.inRangeOf(target) && target.hasSkill("jlsg_zhonghou"))) && !target.isDying();
				});
